...
import { authAPI, getApiError, USE_MOCK_FALLBACK } from '../services/api';
...
export const AccountingProvider = ({ children }) => {
  ...
  const confirmVendorBill = (billId) => {
    ...
    // Automated Double Entry: Dr Purchase Expense, Cr Creditors
  ...
  const registerBillPayment = (billId, paymentMethod = 'BANK') => {
    const bill = vendorBills.find(b => b.id === billId);
    if (!bill) return;
