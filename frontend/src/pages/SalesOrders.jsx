...
export const SalesOrders = () => {
  const { salesOrders, contacts: mockContacts, products: mockProducts, createSalesOrder, confirmSalesOrder, cancelSalesOrder, createInvoiceFromSO, customerInvoices, syncAllData } = useAccounting();
  ...
  const goDetail = (soId) => setSearchParams({ view: 'detail', id: String(soId) });
  const goForm = () => setSearchParams({ view: 'form' });
  ...
  // Show only ACTIVE products in dropdowns
  const allProducts = liveProducts.length ? liveProducts : mockProducts;
  ...
  const handleSubmitForm = async (e) => {
    ...
    for (const l of lines) {
      ...
      if (p && p.type === 'GOODS' && p.stock !== undefined && p.stock < Number(l.quantity)) {
        setFormError(`Insufficient stock for "${p.name}": Available ${p.stock}, Requested ${l.quantity}.`);
        return;
      }
    }
    setBusy(true);
  ...
  const doConfirmSO = async (id) => {
    setActionError('');
    setBusy(true);
    try { await confirmSalesOrder(id); } catch (err) { setActionError(err?.message || 'Confirm failed.'); } finally { setBusy(false); }
  };

  const doCancelSO = async (id) => {
    setActionError('');
    if (!window.confirm('Cancel this Sales Order? This will restore stock if it was confirmed.')) return;
    setBusy(true);
    try { await cancelSalesOrder(id); } catch (err) { setActionError(err?.message || 'Cancel failed.'); } finally { setBusy(false); }
  };
  ...
  const doCreateInvoice = async (id) => {
    ...
    try {
      ...
      if (inv) {
        // Navigate to customer invoices with the new invoice
        window.location.href = `/customer-invoices?view=detail&id=${inv.id}`;
      }
    } catch (err) { setActionError(err?.message || 'Create Invoice failed.'); } finally { setBusy(false); }
  };
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-5">
          <h1 className="text-xl font-bold text-slate-900">New Sales Order</h1>
        ...
        <form onSubmit={handleSubmitForm} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div><label className={label}>SO No. (auto)</label><input value="(Auto on save)" disabled className={`${input} bg-slate-50 text-slate-500`} /></div>
            <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {lines.map((l, i) => {
                  const selProd = products.find(x => String(x.id) === String(l.productId));
                  const stockWarn = selProd && selProd.type === 'GOODS' && selProd.stock !== undefined && Number(l.quantity) > selProd.stock;
                  return (
                    <tr key={i} className="border-b border-slate-50 last:border-0">
                      <td className="py-2 pr-2 text-slate-500">{i + 1}.</td>
                      <td className="py-2 pr-2 min-w-[170px]">
                        <select value={l.productId} onChange={(e) => pickProduct(i, e.target.value)} className={input} required>
                          <option value="">Select…</option>
                          {products.map((p) => <option key={p.id} value={p.id}>{p.name} — {inr(p.salesPrice)} (Stock: {p.type === 'GOODS' ? p.stock ?? '—' : 'N/A'})</option>)}
                        </select>
                        {stockWarn && <p className="text-xs text-red-600 mt-0.5">⚠ Only {selProd.stock} in stock</p>}
                      </td>
                      ...
                      <td className="py-2 pr-2 w-20"><input type="number" min="1" value={l.quantity} onChange={(e) => setLine(i, { quantity: e.target.value })} className={`${input}${stockWarn ? ' border-red-400' : ''}`} /></td>
          ...
          <div className="flex justify-end gap-4 text-sm"><span className="text-slate-500">Subtotal {inr(subtotal)}</span><span className="text-slate-500">GST 18% {inr(tax)}</span><b>Total {inr(subtotal + tax)}</b></div>
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={openNew} className="px-5 py-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-50">New</button>
            <button type="submit" disabled={busy} className="px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-semibold">{busy ? 'Saving…' : 'Save (DRAFT)'}</button>
          </div>
        </form>
      </div>
    );
  ...
  if (viewParam === 'detail' && idParam) {
    const so = salesOrders.find((s) => s.id === idParam) || null;
    if (!so) return (
      <div className="max-w-5xl mx-auto px-4 py-8 text-center">
        <p className="text-slate-500 mb-4">Sales Order not found.</p>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
            ...
            <div><p className="text-[11px] uppercase font-bold text-slate-500">Status</p>
              <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-bold ${STATUS_COLORS[so.status] || 'bg-slate-100 text-slate-700'}`}>{so.status}</span>
            </div>
          </div>
          <table className="w-full text-sm">
          ...
          <div className="flex justify-end text-sm"><span className="text-slate-500 mr-2">Total (incl. GST 18%)</span><b>{inr(so.totalAmount)}</b></div>
          ...
          <div className="flex flex-wrap justify-end gap-2 pt-2">
            ...
            {so.status === 'DRAFT' && (
              <>
                <button onClick={() => doConfirmSO(so.id)} disabled={busy} className="px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-semibold">{busy ? 'Confirming…' : 'Confirm Order'}</button>
                <button onClick={() => doCancelSO(so.id)} disabled={busy} className="px-5 py-2 rounded-xl border border-red-300 text-red-700 hover:bg-red-50 text-sm font-semibold flex items-center gap-1.5"><X className="w-3.5 h-3.5" /> Cancel</button>
              </>
            )}
            {so.status === 'CONFIRMED' && !inv && (
              <>
                <button onClick={() => doCreateInvoice(so.id)} disabled={busy} className="px-6 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white text-sm font-semibold">{busy ? 'Creating…' : 'Create Invoice'}</button>
                <button onClick={() => doCancelSO(so.id)} disabled={busy} className="px-5 py-2 rounded-xl border border-red-300 text-red-700 hover:bg-red-50 text-sm font-semibold flex items-center gap-1.5"><X className="w-3.5 h-3.5" /> Cancel</button>
              </>
            )}
            {inv && <Link to={`/customer-invoices?view=detail&id=${inv.id}`} className="px-6 py-2 rounded-xl bg-slate-900 text-white text-sm font-semibold">Open Invoice {inv.invNumber}</Link>}
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {visibleSOs.map((so) => (
              <tr key={so.id} onClick={() => goDetail(so.id)} className="border-b border-slate-50 last:border-0 hover:bg-blue-50/40 cursor-pointer">
                <td className="px-4 py-3 font-bold">{so.soNumber}</td>
                <td className="px-4 py-3">{so.customerName}</td>
                <td className="px-4 py-3 text-slate-600">{so.date}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${STATUS_COLORS[so.status] || 'bg-slate-100 text-slate-700'}`}>{so.status}</span>
                </td>
                <td className="px-4 py-3 text-right font-semibold">{inr(so.totalAmount)}</td>
              </tr>
            ))}
            {visibleSOs.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500">No sales orders found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
