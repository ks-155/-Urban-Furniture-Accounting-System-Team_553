...
export const Budgets = () => {
  ...
  const [form, setForm] = useState({ name: '', periodStart: '', periodEnd: '', responsiblePerson: '', analyticAccount: '', plannedAmount: '', committedAmount: '', achievedAmount: 0 });

  const selected = budgets.find((b) => b.id === selectedId) || null;
  const openRecord = (b) => { setSelectedId(b.id); setView('form'); };
  const openNew = () => {
    setSelectedId(null);
    setForm({ name: '', periodStart: '', periodEnd: '', responsiblePerson: contacts[0]?.name || '', analyticAccount: '', plannedAmount: '', committedAmount: '', achievedAmount: 0 });
    setView('form');
  };
  ...
  const saveNew = (e) => {
    ...
    const nb = {
      ...
      name: form.name, analyticAccount: form.analyticAccount, periodStart: form.periodStart, periodEnd: form.periodEnd,
      responsiblePerson: form.responsiblePerson, plannedAmount: Number(form.plannedAmount || 0),
      committedAmount: Number(form.committedAmount || 0), achievedAmount: 0, status: 'DRAFT', revisedFrom: null, revisedTo: null,
    };
    setBudgets([nb, ...budgets]);
    setSelectedId(nb.id);
  };

  const confirmBudget = () => setBudgets(budgets.map((b) => (b.id === selectedId ? { ...b, status: 'CONFIRMED' } : b)));

  const reviseBudget = () => {
    const orig = budgets.find((b) => b.id === selectedId);
    if (!orig || orig.status !== 'CONFIRMED') return;
    const nb = {
      ...orig, id: Math.max(0, ...budgets.map((b) => b.id)) + 1,
      name: `${orig.name} Revised`, status: 'DRAFT', revisedFrom: orig.id, revisedTo: null,
    };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            {selected.status === 'DRAFT' && <button onClick={confirmBudget} className="px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold">Confirm</button>}
            {selected.status === 'CONFIRMED' && <button onClick={reviseBudget} className="px-6 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold">Revise</button>}
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form onSubmit={saveNew} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div><label className={label}>Analytic Account</label><input value={form.analyticAccount} onChange={(e) => setForm({ ...form, analyticAccount: e.target.value })} placeholder="e.g. Manufacturing & Procurement" className={input} required /></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" className="px-6 py-2 rounded-xl bg-blue-600 text-white text-sm font-semibold">Confirm</button>
          </div>
        </form>
      </div>
    );
