...
// Live-first Budgets with full lifecycle: DRAFT -> CONFIRMED -> REVISED (+ new DRAFT revision).
// Falls back to local mock flow when the backend is unreachable.
const normLive = (b) => ({
  id: b.id,
  name: b.name || b.budgetName || 'Budget',
  analyticAccount: typeof b.analyticAccount === 'string' ? b.analyticAccount : (b.analyticAccount?.name || '-'),
  ...
  plannedAmount: Number(b.plannedAmount ?? b.planned ?? 0),
  committedAmount: Number(b.committedAmount ?? b.committed ?? 0),
  achievedAmount: Number(b.achievedAmount ?? b.achieved ?? 0),
  achievedPercent: b.achievedPercent,
  amountToAchieve: b.amountToAchieve,
  status: b.status,
});
...
export const Budgets = () => {
  ...
  const [form, setForm] = useState({ name: '', periodStart: '', periodEnd: '', responsiblePerson: '', analyticAccountId: '', plannedAmount: '', committedAmount: '' });
  ...
  const loadBudgets = useCallback(async () => {
    try {
      ...
      const arr = res.data?.budgets || [];
      setBudgets(arr.map(normLive));
      setLive(true);
      return true;
    } catch {
      setLive(false);
  ...
  useEffect(() => {
    analyticsAPI.list().then((res) => setAnalytics(res.data?.analyticAccounts || [])).catch(() => setAnalytics([]));
  }, []);
  ...
  const openNew = () => {
    setSelectedId(null);
    setForm({ name: '', periodStart: '', periodEnd: '', responsiblePerson: contacts[0]?.name || '', analyticAccountId: analytics[0] ? String(analytics[0].id) : '', plannedAmount: '', committedAmount: '' });
    setActionError('');
    setView('form');
  };
  ...
  const saveNew = async (e) => {
    ...
    try {
      const res = await budgetsAPI.create({
        ...
        analyticAccountId: form.analyticAccountId ? Number(form.analyticAccountId) : undefined,
      ...
      if (err?.response?.data?.error) {
        ...
        const nb = {
          ...
          name: form.name, analyticAccount: analytics.find((a) => String(a.id) === String(form.analyticAccountId))?.name || '',
          periodStart: form.periodStart, periodEnd: form.periodEnd, responsiblePerson: form.responsiblePerson,
          plannedAmount: Number(form.plannedAmount || 0), committedAmount: Number(form.committedAmount || 0),
          achievedAmount: 0, status: 'DRAFT', revisedFrom: null, revisedTo: null,
        };
  ...
  const confirmBudget = async () => {
    ...
    try {
      ...
      else setBudgets(budgets.map((b) => (b.id === selectedId ? { ...b, status: 'CONFIRMED' } : b)));
    } finally {
      setBusy(false);
    }
  };
  ...
  const reviseBudget = async () => {
    ...
    try {
      ...
      await loadBudgets();
      // Select the spawned revision (name + " Revised", newest DRAFT)
      const created = res.data?.revisedBudget || res.data?.budget;
      ...
      if (err?.response?.data?.error) {
        ...
        if (orig && orig.status === 'CONFIRMED') {
          const nb = { ...orig, id: Math.max(0, ...budgets.map((b) => b.id)) + 1, name: `${orig.name} Revised`, status: 'DRAFT', revisedFrom: orig.id, revisedTo: null };
  ...
  // After a live revise, jump to the newest "Name Revised" DRAFT record
  useEffect(() => {
    if (!live || view !== 'form' || !selectedId) return;
    const sel = budgets.find((b) => b.id === selectedId);
    if (sel && sel.status === 'REVISED') {
      const rev = [...budgets].reverse().find((b) => b.name === `${sel.name} Revised` && b.status === 'DRAFT');
      if (rev) setSelectedId(rev.id);
    }
  }, [budgets, live, view, selectedId]);
  ...
  const stageColor = (s) => (s === 'CONFIRMED' ? 'bg-emerald-50 text-emerald-700' : s === 'REVISED' ? 'bg-purple-50 text-purple-700' : s === 'SUBMITTED' ? 'bg-amber-50 text-amber-700' : s === 'CANCELLED' ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-700');

  if (view === 'form' && selected) {
    const c = Number(selected.committedAmount || 0), a = Number(selected.achievedAmount || 0);
    const achievedPct = selected.achievedPercent !== undefined && selected.achievedPercent !== null ? `${Number(selected.achievedPercent).toFixed(1)}%` : pct(a, c);
    const toAchieve = selected.amountToAchieve !== undefined && selected.amountToAchieve !== null ? selected.amountToAchieve : c - a;
    const revisedLink = selected.revisedTo ? budgets.find((b) => b.id === selected.revisedTo) : budgets.find((b) => b.name === `${selected.name} Revised`);
    const origLink = selected.revisedFrom ? budgets.find((b) => b.id === selected.revisedFrom) : budgets.find((b) => selected.name.endsWith(' Revised') && b.name === selected.name.replace(/ Revised$/, ''));
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            {selected.status === 'DRAFT' && <button onClick={confirmBudget} disabled={busy} className="px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-semibold">{busy ? 'Confirming...' : 'Confirm'}</button>}
            {selected.status === 'CONFIRMED' && <button onClick={reviseBudget} disabled={busy} className="px-6 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 disabled:opacity-60 text-white text-sm font-semibold">{busy ? 'Revising...' : 'Revise'}</button>}
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form onSubmit={saveNew} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div>
            <label className={label}>Analytic Account (Many2one)</label>
            <select value={form.analyticAccountId} onChange={(e) => setForm({ ...form, analyticAccountId: e.target.value })} className={input}>
              <option value="">Default (first analytic)</option>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={busy} className="px-6 py-2 rounded-xl bg-blue-600 disabled:opacity-60 text-white text-sm font-semibold">{busy ? 'Saving...' : 'Confirm'}</button>
          </div>
        </form>
      </div>
    );
          ...
          <tbody>
            {budgets.map((b) => (
              <tr key={b.id} onClick={() => openRecord(b)} className="border-b border-slate-50 last:border-0 hover:bg-blue-50/40 cursor-pointer">
                ...
                <td className="px-4 py-3 text-right font-semibold">{b.status === 'DRAFT' ? '-' : (b.achievedPercent !== undefined && b.achievedPercent !== null ? `${Number(b.achievedPercent).toFixed(1)}%` : pct(b.achievedAmount, b.committedAmount))}</td>
              </tr>
            ))}
          </tbody>
        </table>
